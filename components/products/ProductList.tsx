...
const ProductList = ({ products, title }: ProductListProps) => {
	return (
		<section className="py-6 sm:py-8">
			...
			<div className="grid grid-cols-2 gap-4 my-8 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">{products.length ? products.map((product: any) => <ProductCard product={product} key={product.id} />) : <p className="font-bold">No Products</p>}</div>
		</section>
	);
};

...
const AccordionTrigger = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Trigger>>(({ className, children, ...props }, ref) => (
	<AccordionPrimitive.Header className="flex">
		<AccordionPrimitive.Trigger
			ref={ref}
			className={cn('flex flex-1 items-center text-left py-4 font-medium transition-all hover:text-emerald-400 duration-200 [&[data-state=open]>svg]:rotate-180 text-sm leading-6 lg:text-base lg:leading-8', className)}
			{...props}
		>
			{children}
